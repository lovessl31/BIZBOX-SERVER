...
DEV_MAIN_DB_PATH = os.path.join(MAIN_DB_PATH1, "dev_bizbox.db")
current_datetime = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
db_path = [MAIN_DB_PATH, DEV_MAIN_DB_PATH]

def create_bzb_db(db_path):
    ...
    for path in db_path:
        ...
        # 사용자 및 회사 정보 삽입
        # 개발 테스트 환경일때만 더미데이터 정보 삽입
        if path == DEV_MAIN_DB_PATH:
...
# DB 생성
if not os.path.exists("bizbox.db") or not os.path.exists("dev_bizbox.db"):
    create_bzb_db(db_path)
else:
